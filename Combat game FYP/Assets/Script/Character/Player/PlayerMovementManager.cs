...
public class PlayerMovementManager : CharacterMovementManager
{
    ...
    private void CalculateVelocity()
    {
        ...
        if(player.characterController.isGrounded && verticalVelocity < 0 && !isJumping)
        {
            verticalVelocity = -2f;
        }
        else if(!isJumping)
    ...
    public void OnEnableDodgeMovement()
    {
        dodgeVelocity = dodgeDirection * dodgeSpeed;
        isDodging = true;
    }

    public void OnDisableDodgeMovement()
    {
    ...
    private void HandleJumpMovement()
    {
        ...
        jumpDirection = (4 * lastMoveDirection.normalized) + Vector3.up;
        player.playerAnimatorManager.PlayTargetActionAnimation("Jump", true, 2, true);
    }

    public void OnEnableJumpMovement()
    {
        jumpVelocity = jumpDirection * jumpHeight;
        isJumping = true;
    }

    public void OnDisableJumpMovement()
    {
    ...
    {//&& PlayerInput.Instance.dodgeInput
        if(player.isPerformingAction && player.animator.applyRootMotion)
        {
            ...
            if (isJumping)
            {
                player.characterController.Move(jumpVelocity * Time.deltaTime);
            }
        }
    }
}
